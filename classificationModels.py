...
# TESTING: split data into training and testing (80/20 ratio)
...
# train model using SVM with linear kernel
svm_linear = SVC(kernel='linear') # using linear kernel
svm_linear.fit(X_train, y_train)

# train model using SVM with polynomial kernel
...
# PREDICTION:
y_pred_dtree = dtree.predict(X_test)	 			# predict using the trained Decision Tree model
y_pred_knn = knn.predict(X_test) 					# predict using the trained KNN model
y_pred_svm_linear = svm_linear.predict(X_test) 		# predict using the trained SVM with linear kernel model
y_pred_svm_poly = svm_poly.predict(X_test) 			# predict using the trained SVM with polynomial kernel model
y_pred_rfm = rfm.predict(X_test) 					# predict using the trained Random Forest model
...
for feature_name in feature_names:
	if feature_name != 'V4':
		...
		print()
		if attribute.lower() not in [attr.lower() for attr in list_attribute]: # convert list to lowercase for comparison
...
input_feature = pd.get_dummies(df_inputs, columns = ['V2', 'V3', 'V5', 'V6', 'V7', 'V8', 'V9', 'V10', 'V11', 'V12', 'V13'])
